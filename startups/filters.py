def filter_startups_for_specialist(queryset, params):
    ...
    if required_profession:
        queryset = queryset.filter(
            required_specialists__profession_id=required_profession
        )

    if required_skills:
        for skill_id in required_skills:
            queryset = queryset.filter(
                required_specialists__skills__id=skill_id
            )
...
def filter_startups_for_investor(queryset, params):
    ...
    if investment_needed_min:
        queryset = queryset.filter(
            investment_needed__gte=investment_needed_min
        )

    if investment_needed_max:
        queryset = queryset.filter(
            investment_needed__lte=investment_needed_max
        )
