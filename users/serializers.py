...
class SpecialistCardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SpecialistProfile
        fields = [
            'user_id', 'full_name', 'avatar',
            'profession', 'bio', 'skills',
            'is_favorited'
        ]
    ...
    def get_is_favorited(self, obj):
        ...
        return Favorite.objects.filter(
            user=request.user, specialist=obj
        ).exists()
...
class SpecialistProfileSerializer(serializers.ModelSerializer):
    ...
    contact_phone = serializers.CharField(
        source='user.contact_phone',
        required=False,
        allow_blank=False,
        allow_null=False
    )
    contact_email = serializers.EmailField(
        source='user.contact_email',
        required=False,
        allow_blank=False,
        allow_null=False
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            'bio', 'contact_phone', 'contact_email',
            'avatar',
    ...
    def update(self, instance, validated_data):
        ...
        instance.profession = validated_data.get(
...
class FounderProfileSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if 'experiences' in validated_data:
            experiences_data = validated_data.get(
                'experiences', instance.experiences
            )
            instance.experiences.all().delete()
            for exp in experiences_data:
...
class InvestorCardSerializer(serializers.ModelSerializer):
    ...
    def get_is_favorited(self, obj):
        ...
        if not request or not request.user.is_authenticated:
            return False
        return Favorite.objects.filter(
            user=request.user, investor=obj
        ).exists()
...
class InvestorProfileSerializer(serializers.ModelSerializer):
    ...
    experience = InvestorPreviousInvestmentSerializer(
        many=True,
        source='previous_investments',
        required=False
    )
    industry = IndustrySerializer(read_only=True)
    industry_id = serializers.PrimaryKeyRelatedField(
    ...
    class Meta:
        ...
        fields = [
            'user_id', 'role', 'full_name',
            'industry', 'industry_id', 'company', 'position',
    ...
    def validate(self, data):
        ...
        invalid_stages = [
            stage for stage in preferred_stages if stage not in valid_stages
        ]
        if invalid_stages:
            raise serializers.ValidationError({
                'preferred_stages':
    ...
    def update(self, instance, validated_data):
        ...
        instance.preferred_stages = validated_data.get(
            'preferred_stages', instance.preferred_stages
        )
        instance.investment_min = validated_data.get(
            'investment_min', instance.investment_min
        )
        instance.investment_max = validated_data.get(
            'investment_max', instance.investment_max
        )
        ...
        if 'previous_investments' in validated_data:
            ...
            for exp in previous_data:
                InvestorPreviousInvestment.objects.create(
                    investor=instance, **exp
                )
