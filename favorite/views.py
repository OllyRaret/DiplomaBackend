...
class FavoriteViewSet(viewsets.ModelViewSet):
    ...
    def specialists(self, request):
        favorites = self.get_queryset().filter(specialist__isnull=False).select_related('specialist__user', 'specialist__profession').prefetch_related('specialist__skills')
        serializer = SpecialistFavoriteSerializer([f.specialist for f in favorites], many=True)
        return Response(serializer.data)

    @action(detail=False, methods=['get'])
    def investors(self, request):
        favorites = self.get_queryset().filter(investor__isnull=False).select_related('investor__user', 'investor__industry')
        serializer = InvestorFavoriteSerializer([f.investor for f in favorites], many=True)
        return Response(serializer.data)
    ...
    def startup_for_specialist(self, request):
        ...
        serializer = StartupForSpecialistShortSerializer([f.startup for f in favorites], many=True)
        return Response(serializer.data)
    ...
    def startup_for_investor(self, request):
        ...
        serializer = StartupForInvestorShortSerializer([f.startup for f in favorites], many=True)
        return Response(serializer.data)
